...
func process_event():	
	if event_manager.battle_finished:
		if (current_state == states.CAT_TURN):
			update_state(states.WON if game_pad.get_result_is_real_word() else states.LOST)
			interactive_dialogue.visible = true
			interactive_dialogue.set_dialogue(event_manager.get_win_dialogue() if game_pad.get_result_is_real_word() else event_manager.get_lose_dialogue())
		else:
			update_state(states.CAT_TURN)
	elif event_manager.event_is_battle():
#		print(game_pad.is_game_complete(), not game_pad.get_result_is_real_word(), not current_state == states.CAT_TURN)
